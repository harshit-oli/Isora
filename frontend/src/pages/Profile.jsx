    ...
    const handleProfile = async () => {
        try {
            const result = await axios.get(`${serverUrl}/api/user/getProfile/${userName}`, {withCredentials: true})
             console.log("saved[0]:", result.data.user?.saved[0]);
            dispatch(setProfileData(result.data.user));

            // Apna profile hai toh saved posts initialize karo
            if (result?.data.user?._id === userData?._id) {
                dispatch(setSavedPosts(result?.data?.user?.saved || []));
            }
        } catch (error) {
            console.log(error);
        }
        ...
        <div className='w-full min-h-screen bg-black'>
            <div className='w-full h-[80px] flex justify-between items-center px-[30px] text-white'>
                <div onClick={() => {navigate("/")}} className='cursor-pointer'>
                    <IoMdArrowBack className='text-white w-[25px] h-[25px]'/>
                </div>
                    ...
                    <div className="flex items-center justify-center gap-[20px]">
                        <div className='flex relative'>
                            {profileData?.followers?.slice(0, 3).map((user, index) => (
                                <div key={index} className={`w-[40px] h-[40px] rounded-full cursor-pointer overflow-hidden ${index > 0 ? "absolute" : ""}`}
                                    style={index > 0 ? {left: `${index * 8}px`} : {}}>
            ...
            <div className='w-full min-h-[100vh] flex justify-center'>
                <div className='w-full max-w-[1000px] flex flex-col items-center rounded-t-[30px] bg-white relative gap-[20px] pt-[30px]'>
                    <div className='w-[100%] max-w-[600px] h-[80px] bg-white rounded-full flex justify-around items-center gap-[10px]'>
                        ...
                        {profileData?._id == userData?._id &&
                            <div className={`${postType == "saved" ? "bg-black shadow-2xl shadow-black text-white" : ""}
                                w-[28%] rounded-full h-[55%] cursor-pointer hover:h-[60%] max-w-[100px] items-center flex justify-center font-semibold`}
                    ...
                    {profileData?._id == userData?._id &&
                        <>
                            ...
                            {/* FIXED: savedPosts Redux state se render karo */}
                            {postType == "saved" && savedPosts?.map((post, index) => (
